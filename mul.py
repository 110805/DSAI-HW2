...
y = np.zeros((len(expected), 6 , len(chars)), dtype=np.bool)
for i, sentence in enumerate(questions):
    x[i] = ctable.encode(sentence, MAXLEN)
for i, sentence in enumerate(expected):
    y[i] = ctable.encode(sentence, 6)
...
model.add(layers.RepeatVector(6))
...
for i in range(len(test_x)):
        x, y = test_x[np.array([i])], test_y[np.array([i])]
